...
/** 
   ...
   * {@inheritdoc}
   */
  public function cron(EntityInterface $entity = NULL, $handler_id = NULL, $schedule_limit = 500000, $send_limit = NULL) {
  ...
  protected function cronSchedule(EntityInterface $entity = NULL, $handler_id = NULL, $limit = 1000) {
    ...
    foreach ($result as $record) {
      ...
      $currentWebFormId = $record->webform_id;
    }
    ...
    // multidimensional array containing webform with handler ids //
    $handler_ids = []; 
    foreach ($webform_ids as $wid) {

      $hids = [];
      $config = \Drupal::config('webform.webform.'.$wid);
      $handlers = $config->get('handlers');
      foreach ($handlers as $handler) {
        array_push($hids,strval($handler['handler_id']));
      };
     $handler_ids += [$wid => $hids];
    }
    $count = 0;
    // Now update all the emails.
    foreach ($records as $record) {
      $count+=1;
      //\Drupal::logger('webformid')->notice(print_r($count,true));
      $record_handler_array = $handler_ids[$record->webform_id];
      if (in_array($record->handler_id,$record_handler_array)) {      
  ...
  protected function cronSend(EntityInterface $entity = NULL, $handler_id = NULL, $limit = 500) {
    ...
    foreach ($result as $record) {
      // get handler ids for webform //
      $currentWebFormId = $record->webform_id;
      $config = \Drupal::config('webform.webform.'.$currentWebFormId);
      $handlers = $config->get('handlers');
      $handler_ids = [];
      foreach ($handlers as $handler) {
        array_push($handler_ids,strval($handler['handler_id']));
      };      



      if (in_array($record->handler_id,$handler_ids)) {  
        $sid = $record->sid;
